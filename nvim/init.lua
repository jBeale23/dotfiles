...
-- Plugins --
-- ------- --
local mini_deps_installed, MiniDeps = pcall(require, "mini.deps") -- Only attempt to add plugins if minideps is available
if not mini_deps_installed then
	vim.notify("[WARN] mini.deps module not found, plugins disabled.", vim.log.levels.WARN)
	return
else
	MiniDeps.setup({})
	...
	add_plugin("mini.statusline", nil,
		{ use_icons = vim.g.have_nerd_font, section_location = function() return "%2l:%-2v" end }) -- Statusline showing warnings, git branch, etc.
	...
	add_plugin("saghen/blink.cmp", "v1.9.1", { -- Allows for autocompletion
		keymap = {
			preset =
			"default" -- By default, <Ctrl-n> for next item, <Ctrl-p> for previous item, <Ctrl-y> to confirm
		},
		appearance = {
